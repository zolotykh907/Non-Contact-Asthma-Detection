...
class DepthKeypointDataset(Dataset):
    def __init__(self, image_dir, keypoints_dir, transform=None, limb_connections=None):
        self.image_dir = image_dir
        self.keypoints_dir = keypoints_dir
        self.transform = transform
        self.limb_connections = limb_connections
        self.image_files = sorted(os.listdir(image_dir))
        self.keypoint_files = sorted(os.listdir(keypoints_dir))
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            depth_image = depth_image.astype(np.float32)  # Преобразуем в float32
            depth_image = self.transform(depth_image)

        # Загрузка ключевых точек
        keypoint_path = os.path.join(self.keypoints_dir, self.keypoint_files[idx])
...
# Загрузка данных
image_dir = '/var/data/pose_estimation/Diploma/neegor/depth_images'  # Директория с изображениями
keypoints_dir = '/var/data/pose_estimation/Diploma/neegor/lables'  # Директория с ключевыми точками
output_image_dir = "/var/data/pose_estimation/Diploma/numpy_saved_img1"

# Создание датасета
dataset = DepthKeypointDataset(image_dir, keypoints_dir, transform=transform, limb_connections=limb_connections)
...
train_loader = DataLoader(train_dataset, shuffle=True, num_workers=4, pin_memory=True)
val_loader = DataLoader(val_dataset, shuffle=False, num_workers=4, pin_memory=True)
...
for epoch in range(num_epochs):
    ...
    for images, keypoints in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        # Очистка памяти
        del images, keypoints, predictions
...
# Сохранение модели
torch.save(model.state_dict(), "numpy_simple_pose_model1.pth")
